...
func Find(xdb *XDB) (count int, res [][]string, err error) {
	...
	err = find0(c, xdb, func(listKey string, datas map[string]interface{}) (err error) {
		...
		if key.Type == Key_Type_KV {
			...
			if !silence {
				if printBytes {
					fmt.Println(listKey, v.String(), ttl, len(v.Bytes()))
				} else {
			...
			if len(key.Selectors) > 0 {
				...
				if !silence {
					if printBytes {
						m, e := c.HgetAll(listKey)
						if e != nil {
							panic(fmt.Errorf("find0 hgetall %s err %s", listKey, e.Error()))
						}
						fmt.Println(listKey, size, CountMapValueBytes(m))
					} else {
